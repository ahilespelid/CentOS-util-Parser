...
-p) PROC_COUNT="$2"
    break ;;
#-u) mysql -uroot nowhenati -e "INSERT INTO comics_vs_character (comics_id, character_id) SELECT b.comics_id, a.id AS character_id FROM (SELECT c.id AS comics_id, (SELECT autors FROM parser_data p WHERE p.post_id=url_id) AS persons FROM comics c WHERE c.id=${2}) AS b INNER JOIN comics_characters a ON b.persons REGEXP CONCAT('[[:<:]]', a.title, '[[:>:]]');"
-u) mysql -uroot nowhenati -e "INSERT INTO comics_vs_character (comics_id, character_id) SELECT b.comics_id, a.id AS character_id FROM (SELECT c.id AS comics_id, (SELECT characters FROM parser_data p WHERE p.post_id=url_id) AS persons FROM comics c WHERE c.id=${2}) AS b INNER JOIN comics_characters a ON (b.persons LIKE CONCAT('%',a.title,'%') OR b.persons LIKE CONCAT('%, ',a.title,'') OR b.persons LIKE CONCAT('',a.title,''));"
