    ...
    then
        ...
        then
            ...
            do
                ...
                mysql -uroot nowhenati -e "INSERT INTO nowhenati.comics_artists SET title='${g}', count_comics=count_comics+1 ON DUPLICATE KEY UPDATE count_comics=count_comics+1; UPDATE comics_artists SET alias = REPLACE(title, ' ', '-') WHERE title='${g}';"
                # echo "INSERT INTO nowhenati.comics_artists SET title='${g}', count_comics=count_comics+1 ON DUPLICATE KEY UPDATE count_comics=count_comics+1;"
                echo "${i} : $g ADD";
            done
        else
            echo "${i} : $COMICS_ARTISTS EXISTS"           
        fi
        echo -e "\n";
    fi
    sleep 0.2
